...
export default {
    ...
    /*methods: {
        updateStatus(product) {
            const status = product.product_status ? 0 : 1;
            axios.get('/changeStatus', {
            params: { product_status: status, product_id: product.id }
            }).then(response => {
    ...
    <AppLayout title="Dashboard">
        <template #header>
            ...
            <div class="flex justify-between">
                ...
                <Link :href="route('products.create')" class="bg-blue-500 text-white font-bold py-2 px-4 rounded-md">
                    Crear
                </Link>
            </div>
        </template>
        <table class="mt-4 min-w-full divide-y divide-gray-200 border">
            ...
            <tr>
                ...
                <th class="px-6 py-3 bg-gray-50 text-left">
                    <span class="text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Cantidad de Productos</span>
                </th>
                <th class="px-6 py-3 bg-gray-50 text-left">
            ...
            <tr v-for="product in products">
                ...
                <!-- <td> 
                    <label class="relative inline-flex items-center mr-5 cursor-pointer">
                        <input type="checkbox" value="" class="sr-only peer" :checked="product.product_status" @change="updateStatus(product)" :data-id="product.id">
                ...
                <td class="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-900">
                    <Link :href="route('products.show', product.id)" class="px-2 py-1 bg-blue-600 text-white rounded font-bold uppercase mr-2">Ver</Link>
                    <Link :href="route('products.edit', product.id)" class="px-2 py-1 bg-blue-600 text-white rounded font-bold uppercase mr-2">Editar</Link>
                    <button  @click="destroy(product.id)" type="button" class="px-2 py-1 bg-red-600 text-white rounded font-bold uppercase">
                        Eliminar
                    </button>
                </td>
            </tr>
